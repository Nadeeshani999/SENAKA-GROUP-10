...
const Sound = {
  ...
  // Urgent alarm for expired tenders — descending tri-tone pulse
  alarm() {
    const pulses=[
      [880,'square',.12,.18,440],
      [880,'square',.12,.18,440],
      [880,'square',.16,.20,300]
    ];
    pulses.forEach(([f,t,d,v,sw],i)=>{
      setTimeout(()=>this._tone(f,t,d,v,sw),i*200);
    });
    // second burst after gap
    setTimeout(()=>{
      this._tone(660,'square',.10,.15,300);
      setTimeout(()=>this._tone(440,'square',.18,.18,220),130);
    },700);
  }
};
  ...
  bar(id, labels, datasets, opts={}) {
    ...
    State.charts[id]=new Chart(canvas,{
      ...
      data:{ labels, datasets:datasets.map((d,i)=>({
        ...
        borderRadius:opts.stacked?0:8, borderSkipped:false,
        maxBarThickness: opts.maxBarThickness || 80,
        ...d
      }))},
      options:cfg
    });
...
async function initFinancePage() {
  ...
  allRows._renderer = r => {
    ...
    return `<tr>
      ...
      <td class="num text-muted">${(r['Age Days'] || 0).toLocaleString()}</td>
...
async function initTenderPage() {
  ...
  function renderView(rows) {
    activeRows=rows;
    const selectedSite=siteEl?.value||'';
    ...
    // ── Donut: always all sites for comparison, highlight selected ──
    const bySiteAll={};
    allRows.forEach(r=>{ const s=r['Site Name']||'Unknown'; bySiteAll[s]=(bySiteAll[s]||0)+1; });
    Charts.donut('tender-donut',Object.keys(bySiteAll),Object.values(bySiteAll),{
      highlight: selectedSite||null
    });

    // ── Polar: always ALL sites so comparison is never lost ──
    const allSiteNames=[...new Set(allRows.map(r=>r['Site Name']).filter(Boolean))];
    const allSiteQty=allSiteNames.map(s=>allRows.filter(r=>r['Site Name']===s).reduce((acc,r)=>acc+(r['Total Tender Qty']||r['Tender Qty']||0),0));
    const allSiteCasted=allSiteNames.map(s=>allRows.filter(r=>r['Site Name']===s).reduce((acc,r)=>acc+(r['Total Qty Casted']||0),0));
    // Highlight selected site in polar chart — dim others
    const polarColors=allSiteNames.map(s=>
      !selectedSite || s===selectedSite
        ? undefined          // use default vibrant color
        : 'rgba(255,255,255,0.10)'  // dimmed
    );
    if (allSiteNames.length) Charts.polar('tender-radar',allSiteNames,allSiteQty,{unit:' poles', colors:polarColors});

    // Update polar chart subtitle to reflect filter context
    const radarSub=document.querySelector('#tender-radar')?.closest('.chart-card')?.querySelector('.chart-card-sub');
    if (radarSub) radarSub.textContent = selectedSite
      ? `All sites compared — "${selectedSite}" highlighted`
      : 'Total quantity comparison per site';

    // siteCasted still needed for gauges — use allRows per site
    const siteCasted=allSiteNames.map(s=>allRows.filter(r=>r['Site Name']===s).reduce((acc,r)=>acc+(r['Total Qty Casted']||0),0));

    /* ── Pole-type contract duration map ── */
    function getPoleLimit(poleType) {
      const pt=(poleType||'').toUpperCase().replace(/[\s\-_]/g,'');
      if (['NWP1','NWP2'].some(x=>pt.includes(x.replace(/[\s\-_]/g,'')))) return 75;
      if (['CP1','CP2'].some(x=>pt.includes(x.replace(/[\s\-_]/g,'')))) return 70;
      if (pt.includes('WPN')) return 90;
      // SP1, SP2, EP, NP and all others default to 60
      return 60;
    }

    /* ── Half-circle expire-age meters — one per ON-GOING tender ── */
    const gaugesEl=document.getElementById('tender-gauges');
    const ongoingForGauges=rows.filter(r=>r._tabStatus==='On-Going');
    let hasExpiredTenders=false;

    if (gaugesEl) {
      if (!ongoingForGauges.length) {
        gaugesEl.innerHTML='<div style="color:rgba(255,255,255,.35);font-size:.75rem;padding:.5rem">No on-going tenders</div>';
      } else {
        gaugesEl.innerHTML=ongoingForGauges.map((r,i)=>{
          const poleType=r['Pole Type']||'';
          const limitDays=getPoleLimit(poleType);
          // "Expire Date Age" is column L — numeric days elapsed since production start
          const ageDays=parseFloat(r['Expire Date Age']||r['Expire Age']||r['Age']||0)||0;
          const usedPct=Math.min(110, ageDays/limitDays*100); // allow over 100 for visual overflow
          const isExpired=ageDays>limitDays;
          const nearExpiry=!isExpired&&(limitDays-ageDays)<=7;
          if (isExpired) hasExpiredTenders=true;

          const displayPct=Math.min(100,usedPct);
          const col=isExpired?C.red:nearExpiry?C.amber:usedPct>=70?C.amber:C.green;

          // Single arc meter — one glowing arc only, no background track
          const cx=44, cy=42, rad=33;
          // Full arc path left→right
          const singleArc=`M ${cx-rad} ${cy} A ${rad} ${rad} 0 0 1 ${cx+rad} ${cy}`;

          const gmId=`gm-exp-${i}`;
          const daysLeft=limitDays-ageDays;

          // Needle angle: 180°=0%, 0°=100%
          const needleAngle=Math.PI-(displayPct/100)*Math.PI;
          const needleLen=rad-5;
          const needleTipX=cx+needleLen*Math.cos(needleAngle);
          const needleTipY=cy-needleLen*Math.sin(needleAngle);
          const arrowSize=3.5;
          const backX=cx+(needleLen-arrowSize*1.8)*Math.cos(needleAngle);
          const backY=cy-(needleLen-arrowSize*1.8)*Math.sin(needleAngle);
          const perpX=Math.sin(needleAngle)*arrowSize;
          const perpY=Math.cos(needleAngle)*arrowSize;
          const arrL=`${(backX-perpX).toFixed(2)},${(backY+perpY).toFixed(2)}`;
          const arrR=`${(backX+perpX).toFixed(2)},${(backY-perpY).toFixed(2)}`;
          const arrTip=`${needleTipX.toFixed(2)},${needleTipY.toFixed(2)}`;
          const needleStem=`M ${cx} ${cy} L ${backX.toFixed(2)} ${backY.toFixed(2)}`;
          const arrowHead=`M ${arrTip} L ${arrL} L ${arrR} Z`;

          return `<div style="text-align:center;min-width:95px;${isExpired?'animation:expiredPulse 1.2s ease-in-out infinite;':''}" title="${r['Tender No']||''} · ${poleType} · Limit: ${limitDays}d">
            <svg viewBox="0 0 88 52" width="95" height="56" style="overflow:visible;display:block;margin:0 auto">
              <defs>
              ...
              <!-- Single glowing arc — no background track -->
              <path d="${singleArc}" fill="none" stroke="${col}" stroke-width="7" stroke-linecap="round" filter="url(#glow-${i})"/>
              <!-- Needle stem — same color as arc -->
              <path d="${needleStem}" stroke="${col}" stroke-width="1.5" stroke-linecap="round" opacity="0.9"/>
              <!-- Needle arrowhead — same color as arc -->
              <path d="${arrowHead}" fill="${col}" style="filter:drop-shadow(0 0 3px ${col})"/>
              <!-- Pivot -->
              <circle cx="${cx}" cy="${cy}" r="2.5" fill="${col}" stroke="rgba(0,0,0,.7)" stroke-width="1"/>
              <!-- Value -->
              <text x="${cx}" y="${cy-7}" text-anchor="middle" font-size="8.5" font-weight="800" fill="${col}" font-family="monospace">+${Math.round(ageDays)}d</text>
            </svg>
            <div style="font-size:.55rem;font-weight:700;color:${col};margin-top:.04rem;letter-spacing:.4px">△ +${Math.round(ageDays)}d</div>
            <div style="font-size:.50rem;color:rgba(255,255,255,.55);margin-top:.1rem;font-weight:600">${r['Tender No']||'—'}</div>
            <div style="font-size:.46rem;color:rgba(255,255,255,.35);margin-top:.05rem">${poleType||'—'} · ${r['Site Name']||'—'}</div>
          </div>`;
        }).join('');

        // Inject expire-pulse CSS animation if not already present
        if (!document.getElementById('expire-pulse-style')) {
          const st=document.createElement('style');
          st.id='expire-pulse-style';
          st.textContent=`
            @keyframes expiredPulse {
              0%,100%{ filter:drop-shadow(0 0 0px #f43f5e); opacity:1; }
              50%{ filter:drop-shadow(0 0 8px #f43f5e); opacity:.82; }
            }
          `;
          document.head.appendChild(st);
        }

        // Play alarm if any on-going tender has exceeded its limit
        if (hasExpiredTenders) {
          setTimeout(()=>Sound.alarm(), 600);
          // Show a toast warning
          const expiredList=ongoingForGauges.filter(r=>{
            const ad=parseFloat(r['Expire Date Age']||r['Expire Age']||r['Age']||0)||0;
            return ad>getPoleLimit(r['Pole Type']||'');
          }).map(r=>r['Tender No']||'Tender').join(', ');
          toast(`⚠ EXPIRED: ${expiredList} — contract time limit exceeded!`,'warn');
        }
      }
    }

    /* ── Build shared max delay for consistent needle scale ── */
    const allDelayVals=rows.map(r=>Math.abs(parseFloat(r['Delays (Expected or Actual)']??r['Delays']??0)||0));
    const maxDelayAbs=Math.max(1,...allDelayVals);

    /* ── Inject delay pulse CSS once ── */
    if (!document.getElementById('delay-meter-style')) {
      const dst=document.createElement('style');
      dst.id='delay-meter-style';
      dst.textContent=`
        @keyframes delayPulse {
          0%,100%{ filter:drop-shadow(0 0 0px #f43f5e); opacity:1; }
          50%{ filter:drop-shadow(0 0 12px #f43f5e); opacity:.80; }
        }
        @keyframes delayPulseAmber {
          0%,100%{ filter:drop-shadow(0 0 0px #f59e0b); opacity:1; }
          50%{ filter:drop-shadow(0 0 10px #f59e0b); opacity:.82; }
        }
        .td-meter { text-align:center; vertical-align:middle; padding:6px 8px !important; }
        .inline-meter-wrap { display:inline-flex; flex-direction:column; align-items:center; gap:2px; }
        .inline-meter-wrap.m-delayed { animation:delayPulse 1.3s ease-in-out infinite; }
      `;
      document.head.appendChild(dst);
    }

    /* ── Single arc meter — one glowing arc only, no background track ── */
    function buildDelayMeter(delayVal, rowIdx) {
      if (isNaN(delayVal)) return '<span style="color:rgba(255,255,255,.22);font-size:.7rem">—</span>';
      const isDelayed=delayVal>0, isAhead=delayVal<0;
      const col=isDelayed?'#f43f5e':isAhead?'#10b981':'#4a5568';
      const fillPct=Math.min(100,(Math.abs(delayVal)/maxDelayAbs)*100);

      const cx=44, cy=42, rad=33;
      const singleArc=`M ${cx-rad} ${cy} A ${rad} ${rad} 0 0 1 ${cx+rad} ${cy}`;
      const filtId=`gf-dl-${rowIdx}`;

      const needleAngle=Math.PI-(fillPct/100)*Math.PI;
      const needleLen=rad-5, arrowSize=3.5;
      const ntx=cx+needleLen*Math.cos(needleAngle), nty=cy-needleLen*Math.sin(needleAngle);
      const bx=cx+(needleLen-arrowSize*1.8)*Math.cos(needleAngle), by=cy-(needleLen-arrowSize*1.8)*Math.sin(needleAngle);
      const px=Math.sin(needleAngle)*arrowSize, py=Math.cos(needleAngle)*arrowSize;
      const needleStem=`M ${cx} ${cy} L ${bx.toFixed(2)} ${by.toFixed(2)}`;
      const arrowHead=`M ${ntx.toFixed(2)},${nty.toFixed(2)} L ${(bx-px).toFixed(2)},${(by+py).toFixed(2)} L ${(bx+px).toFixed(2)},${(by-py).toFixed(2)} Z`;

      const valTxt=`${isDelayed?'+':''}${Math.round(delayVal)}d`;
      const lblTxt=isDelayed?`△ +${Math.round(delayVal)}d`:isAhead?`✔ ${Math.round(Math.abs(delayVal))}d ahead`:`✔ On time`;
      const lblCol=isDelayed?'#f43f5e':isAhead?'#10b981':'#7c8db0';

      return `<div class="inline-meter-wrap${isDelayed?' m-delayed':''}" style="text-align:center;min-width:95px;">
        <svg viewBox="0 0 88 52" width="95" height="56" style="overflow:visible;display:block;margin:0 auto">
          <defs><filter id="${filtId}"><feGaussianBlur stdDeviation="2.5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>
          <path d="${singleArc}" fill="none" stroke="${col}" stroke-width="7" stroke-linecap="round" filter="url(#${filtId})"/>
          <!-- Needle stem — same color as arc -->
          <path d="${needleStem}" stroke="${col}" stroke-width="1.5" stroke-linecap="round" opacity="0.9"/>
          <!-- Needle arrowhead — same color as arc -->
          <path d="${arrowHead}" fill="${col}" style="filter:drop-shadow(0 0 3px ${col})"/>
          <circle cx="${cx}" cy="${cy}" r="2.5" fill="${col}" stroke="rgba(0,0,0,.7)" stroke-width="1"/>
          <text x="${cx}" y="${cy-7}" text-anchor="middle" font-size="8.5" font-weight="800" fill="${col}" font-family="monospace">${valTxt}</text>
        </svg>
        <div style="font-size:.55rem;font-weight:700;color:${lblCol};margin-top:.04rem;letter-spacing:.4px">${lblTxt}</div>
      </div>`;
    }


    let hasDelayedRows=false;
    const delayedTenderNames=[];
    ...
    rows._renderer=r=>{
      ...
      const delayVal=parseFloat(r['Delays (Expected or Actual)']??r['Delays']??'');
      const hasDelay=!isNaN(delayVal);
      const isDelayed=hasDelay&&delayVal>0;
      const isAhead=hasDelay&&delayVal<0;
      const delayCls=isDelayed?'text-red':isAhead?'text-green':'text-muted';

      if (isDelayed) { hasDelayedRows=true; delayedTenderNames.push(r['Tender No']||'Tender'); }

      const rowIdx=rows.indexOf(r);
      const meterCell = buildDelayMeter(delayVal, rowIdx);

      return `<tr${isDelayed?' class="row-expired"':''}>
        <td><button class="expand-btn">▶</button></td>
        <td>${r['Tender No']||'—'}</td>
        <td>${r['Site Name']||'—'}</td>
        <td>${r['Pole Type']||'—'}</td>
        ...
        <td class="td-meter">${meterCell}</td>
      </tr><tr class="expand-row" style="display:none"><td colspan="9">
        <div class="expand-content">
          ...
          <div class="expand-field"><div class="expand-field-label">Delays</div><div class="expand-field-val ${delayCls}">${hasDelay?Math.round(delayVal)+' days '+(isDelayed?'— ⚠ DELAYED':isAhead?'— ahead of schedule':'— on schedule'):'—'}</div></div>
          ${isOng?`
    ...
    if (hasDelayedRows) {
      setTimeout(()=>Sound.warn(), 800);
      toast(`⚠ DELAY ALERT: ${delayedTenderNames.join(', ')} — production behind schedule!`,'warn');
    }
  }
...
async function initFillingPage() {
  ...
  /* ── Flexible column resolver ──────────────────────────────────────
     Google Sheets sometimes exports column names with inconsistent
     spacing or casing. This finds the best matching key in a row. */
  function colKey(row, ...candidates) {
    ...
    for (const c of candidates) {
      ...
      const found = keys.find(k => k.replace(/\s+/g,' ').trim().toLowerCase() === norm);
      if (found) return found;
    }
    return candidates[0]; // default to first candidate (will return null/0 safely)
  }

  /* Resolve the actual column names once from the first row that has data */
  const sampleRow = rows.find(r => Object.values(r).some(v => v !== null)) || latest;
  const COL = {
    petrolLKR:    colKey(sampleRow, 'Petrol Sales (LKR)', 'Petrol Sales(LKR)', 'Petrol Sales (Rs.)'),
    dieselLKR:    colKey(sampleRow, 'Diesel Sales (LKR)', 'Diesel Sales(LKR)', 'Diesel Sales (Rs.)'),
    euro3LKR:     colKey(sampleRow, 'Euro 3 Sales (LKR)', 'Euro 3 Sales(LKR)', 'Euro3 Sales (LKR)', 'Euro 3 Sales (Rs.)'),
    extraMileLKR: colKey(sampleRow, 'Extra Mile Sales (LKR)', 'Extra Mile Sales(LKR)', 'Extra Mile Sales (Rs.)'),
    petrolL:      colKey(sampleRow, 'Petrol Sales (L)', 'Petrol Sales(L)', 'Petrol Volume (L)'),
    dieselL:      colKey(sampleRow, 'Diesel Sales (L)', 'Diesel Sales(L)', 'Diesel Volume (L)'),
    euro3L:       colKey(sampleRow, 'Euro 3 Sales (L)', 'Euro 3 Sales(L)', 'Euro3 Sales (L)'),
    extraMileL:   colKey(sampleRow, 'Extra Mile Sales (L)', 'Extra Mile Sales(L)'),
    petrolStock:  colKey(sampleRow, 'Petrol Stock'),
    dieselStock:  colKey(sampleRow, 'Diesel Stock'),
    euro3Stock:   colKey(sampleRow, 'Euro 3 Stock', 'Euro3 Stock'),
    emStock:      colKey(sampleRow, 'Extra Mile Stock'),
    creditSales:  colKey(sampleRow, 'Credit Sales'),
    cashSales:    colKey(sampleRow, 'Cash Sales'),
    cardSales:    colKey(sampleRow, 'Card Sales'),
    bankTransfer: colKey(sampleRow, 'Bank Transfers', 'Bank Transfer'),
  };

  /* ── Tank levels ───────────────────────────────────────────────── */
  const TANK_MAX={petrol:20000,diesel:30000,euro3:15000,extramile:10000};
  [
    {id:'petrol',   col:COL.petrolStock},
    {id:'diesel',   col:COL.dieselStock},
    {id:'euro3',    col:COL.euro3Stock},
    {id:'extramile',col:COL.emStock}
  ].forEach(t=>{
  ...
  /* ── KPI totals — all four fuel types included ─────────────────── */
  const totRev = rows.reduce((s,r) =>
    s + (r[COL.petrolLKR]||0) + (r[COL.dieselLKR]||0) +
        (r[COL.euro3LKR]||0)  + (r[COL.extraMileLKR]||0), 0);

  const totVol = rows.reduce((s,r) =>
    s + (r[COL.petrolL]||0) + (r[COL.dieselL]||0) +
        (r[COL.euro3L]||0)  + (r[COL.extraMileL]||0), 0);

  const creditBal = rows.reduce((s,r) => s + (r[COL.creditSales]||0), 0);
  ...
  /* ── Charts ────────────────────────────────────────────────────── */
  const fuelLKRCols  = [COL.petrolLKR,  COL.dieselLKR,  COL.euro3LKR,  COL.extraMileLKR];
  const fuelLCols    = [COL.petrolL,    COL.dieselL,    COL.euro3L,    COL.extraMileL];
  const fuelLabels   = ['Petrol','Diesel','Euro 3','Extra Mile'];

  const revVals = fuelLKRCols.map(col => rows.reduce((s,r)=>s+(r[col]||0),0));
  Charts.bar('fill-rev',fuelLabels,[{label:'Revenue (LKR)',data:revVals,colors:[C.blue+'cc',C.amber+'cc',C.green+'cc',C.red+'cc']}],{currency:true});

  const sTypesCols  = [COL.cashSales, COL.cardSales, COL.bankTransfer, COL.creditSales];
  const sTypesLabels= ['Cash Sales','Card Sales','Bank Transfers','Credit Sales'];
  Charts.donut('fill-sales',sTypesLabels,sTypesCols.map(k=>latest[k]||0),{currency:true});

  const volVals = fuelLCols.map(col => rows.reduce((s,r)=>s+(r[col]||0),0));
  Charts.bar('fill-vol',fuelLabels,[{label:'Volume (L)',data:volVals,colors:[C.blue+'cc',C.amber+'cc',C.green+'cc',C.red+'cc']}],{unit:' L'});

  const cb={'0–30d':0,'31–60d':0,'61–90d':0,'91+d':0};
  rows.forEach(r=>{ const age=daysAgo(r['Date'])||0, cr=r[COL.creditSales]||0;
  ...
  /* ── Table renderer — all 4 fuel types + expand detail row ─────── */
  rows._renderer = r => {
    const pL   = r[COL.petrolL]||0;
    const dL   = r[COL.dieselL]||0;
    const e3L  = r[COL.euro3L]||0;
    const emL  = r[COL.extraMileL]||0;
    const pLKR = r[COL.petrolLKR]||0;
    const dLKR = r[COL.dieselLKR]||0;
    const e3LKR= r[COL.euro3LKR]||0;
    const emLKR= r[COL.extraMileLKR]||0;
    const totVL = pL + dL + e3L + emL;
    const totRL = pLKR + dLKR + e3LKR + emLKR;
    const cash  = r[COL.cashSales]||0;
    const card  = r[COL.cardSales]||0;
    const bank  = r[COL.bankTransfer]||0;
    const credit= r[COL.creditSales]||0;
    return `<tr>
      <td><button class="expand-btn">▶</button></td>
      <td>${fmtDate(r['Date'])}</td>
      <td class="num">${pL.toFixed(2)}</td>
      <td class="num">${dL.toFixed(2)}</td>
      <td class="num">${e3L.toFixed(2)}</td>
      <td class="num">${emL.toFixed(2)}</td>
      <td class="num">Rs. ${fmtLKR(pLKR)}</td>
      <td class="num">Rs. ${fmtLKR(dLKR)}</td>
      <td class="num">Rs. ${fmtLKR(e3LKR)}</td>
      <td class="num">Rs. ${fmtLKR(emLKR)}</td>
    </tr><tr class="expand-row" style="display:none"><td colspan="10">
      <div class="expand-content">
        <div class="expand-field"><div class="expand-field-label">Cash Sales</div><div class="expand-field-val">Rs. ${fmtLKR(cash)}</div></div>
        <div class="expand-field"><div class="expand-field-label">Card Sales</div><div class="expand-field-val">Rs. ${fmtLKR(card)}</div></div>
        <div class="expand-field"><div class="expand-field-label">Bank Transfers</div><div class="expand-field-val">Rs. ${fmtLKR(bank)}</div></div>
        <div class="expand-field"><div class="expand-field-label">Credit Sales</div><div class="expand-field-val">Rs. ${fmtLKR(credit)}</div></div>
        <div class="expand-field"><div class="expand-field-label">Total Volume (L)</div><div class="expand-field-val">${totVL.toFixed(2)} L</div></div>
        <div class="expand-field"><div class="expand-field-label">Total Revenue</div><div class="expand-field-val text-green">Rs. ${fmtLKR(totRL)}</div></div>
      </div>
    </td></tr>`;
  };
